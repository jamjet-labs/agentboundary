...
**Canonicalization is critical for portability.** Two correct implementations MUST produce the same `arguments_hash` for the same logical arguments. Canonicalization MUST follow [JSON Canonicalization Scheme (RFC 8785)](https://tools.ietf.org/html/rfc8785) — in particular its number serialization (ECMAScript `Number.prototype.toString`, so an integer-valued number such as `50.0` serializes as `50`) and its key ordering (lexicographic by UTF-16 code unit). An ad-hoc scheme such as `json.dumps(sort_keys=True)` is NOT conformant: it disagrees with RFC 8785 on integer-valued floats and on key ordering for code points outside the Basic Multilingual Plane, so two implementations would compute different hashes for the same receipt.

> **Changed in v0.1.1:** v0.1.0 only RECOMMENDED RFC 8785. It is now mandatory. The reference implementation prior to this change used an ad-hoc canonicalizer that diverged from RFC 8785 on the two cases above; see the `hashing` module changelog.
...
- ~~Mandatory canonicalization scheme (RFC 8785 candidate)~~ — **resolved in v0.1.1: RFC 8785 is now mandatory (§4.8).**
...
### 7.2 What's the canonical canonicalization? — RESOLVED (v0.1.1)

**Resolved:** RFC 8785 (JSON Canonicalization Scheme) is now mandatory for `arguments_hash` and `receipt_hash` (§4.8). The earlier "RECOMMENDED but not mandated" stance allowed different implementations to produce different hashes for the same logical content — specifically, an ad-hoc `json.dumps(sort_keys=True)` diverges from RFC 8785 on integer-valued floats (`50.0` vs `50`) and on key ordering for non-BMP code points. Mandating RFC 8785 closes that gap.
